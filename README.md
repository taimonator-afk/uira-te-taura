# Uira – Te Taura Whakamātau Reanga 9  
### Part of the *Te Reo Makitaunu (TRM)* Programme

---

## 🌿 Overview
**Uira** is an interactive bilingual (Te Reo Māori + English) assessment tool created for the *Te Reo Makitaunu (TRM)* language learning framework.  
It enables learners to complete structured assessments, receive automatic scoring, and download personalised PDF reports.  

This version is optimised for use within **Systeme.io** and other approved TRM learning platforms.

---

## 🧩 Features
- 🪶 Fully bilingual interface (Te Reo Māori + English)  
- 🔀 Randomised multiple-choice questions  
- 📄 Auto-generated PDF reports (landscape format)  
- 🖼 TRM-branded layout with logo and copyright  
- 🧠 Aligned to TRM *Reanga* curriculum standards  
- 🔒 Protected by TRM proprietary licensing  

---

## ⚙️ Installation & Hosting

### **Option 1 – GitHub Pages (recommended)**
1. Upload this repository to your GitHub account.  
2. Go to **Settings → Pages → Source → main branch** → Save.  
3. After publishing, your site will be live at:  
